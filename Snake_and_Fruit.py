...
class SnakeAndFruit:
    ...
    def __init__(self):
        self.window = Tk()
        self.window.title("Snake-and-Fruit")
    ...
    def place_Fruit(self):
        # Place Fruit randomly anywhere except at the cells occupied by snake
        unoccupied_cels = set(self.board) - set(self.snake)
        self.Fruit_cell = random.choice(list(unoccupied_cels))
        row_h = int(size_of_board / rows)
        col_w = int(size_of_board / cols)
        x1 = self.Fruit_cell[0] * row_h
        y1 = self.Fruit_cell[1] * col_w
        x2 = x1 + row_h
        y2 = y1 + col_w
        self.Fruit_obj = self.canvas.create_rectangle(
    ...
    def display_snake(self, mode=""):
        ...
        if mode == "complete":
            ...
            if self.snake[0] == self.old_Fruit_cell:
                self.snake.insert(0, self.old_Fruit_cell)
                self.old_Fruit_cell = []
                tail = self.snake[0]
                row_h = int(size_of_board / rows)
                col_w = int(size_of_board / cols)
    ...
    def update_snake(self, key):
        ...
        if (
            ...
            self.crashed = True
        elif self.Fruit_cell == head:
            # Got the Fruit
            self.old_Fruit_cell = self.Fruit_cell
            self.canvas.delete(self.Fruit_obj)
            self.place_Fruit()
            self.display_snake()
        else:
